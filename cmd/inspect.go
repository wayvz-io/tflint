...
func (cli *CLI) inspectModule(opts Options, dir string, filterFiles []string) (tflint.Issues, map[string][]byte, error) {
	...
	rulesetPlugin, updatedConfig, err := launchPlugins(cli.config, opts.Fix)
	if updatedConfig != nil {
		cli.config = updatedConfig // Use the potentially re-parsed config
	}
...
func launchPlugins(config *tflint.Config, fix bool) (*plugin.Plugin, *tflint.Config, error) {
	// Lookup plugins
	...
	if err != nil {
		return nil, nil, fmt.Errorf("Failed to apply compatibility shim: %w", err)
	}
	...
	for name, ruleset := range rulesetPlugin.RuleSets {
		...
		if err != nil {
			if st, ok := status.FromError(err); ok && st.Code() == codes.Unimplemented {
				...
				return rulesetPlugin, config, fmt.Errorf(`Plugin "%s" SDK version is incompatible. Compatible versions: %s`, name, plugin.SDKVersionConstraints)
			} else {
				return rulesetPlugin, config, fmt.Errorf(`Failed to get TFLint version constraints to "%s" plugin; %w`, name, err)
			}
		}
		if !constraints.Check(tflint.Version) {
			return rulesetPlugin, config, fmt.Errorf("Failed to satisfy version constraints; tflint-ruleset-%s requires %s, but TFLint version is %s", name, constraints, tflint.Version)
		}

		if err := ruleset.ApplyGlobalConfig(pluginConf); err != nil {
			return rulesetPlugin, config, fmt.Errorf(`Failed to apply global config to "%s" plugin; %w`, name, err)
		}
		configSchema, err := ruleset.ConfigSchema()
		if err != nil {
			return rulesetPlugin, config, fmt.Errorf(`Failed to fetch config schema from "%s" plugin; %w`, name, err)
		}
		...
		if plugin, exists := config.Plugins[name]; exists {
			...
			if diags.HasErrors() {
				return rulesetPlugin, config, fmt.Errorf(`Failed to parse "%s" plugin config; %w`, name, diags)
			}
		}
		err = ruleset.ApplyConfig(content, config.Sources())
		if err != nil {
			return rulesetPlugin, config, fmt.Errorf(`Failed to apply config to "%s" plugin; %w`, name, err)
		}
	...
	if err := config.ValidateRules(rulesets...); err != nil {
		return rulesetPlugin, config, fmt.Errorf("Failed to check rule config; %w", err)
	}

	return rulesetPlugin, config, nil
}
